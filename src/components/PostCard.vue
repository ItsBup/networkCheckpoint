    ...
    <div class="card-body d-flex justify-content-start">
      <div class="rounded-circle">
        <router-link :to="{ name: 'Profile', params: { profileId: post.creatorId } }">
          <img :title="`Click to go to ${post.creator.name}'s Profile Page :D`" class="profile-pic"
            :src="post.creator.picture" :alt="post.creator.name">
        </router-link>
      </div>
      <div class="ms-3 mb-4">
      ...
      <p v-if="post.creator.graduated" class="card-subtitle text-body-secondary"><i
          class="mdi mdi-account-school-outline"></i></p>
      <div>
      </div>
    </div>
    <img :src="post.imgUrl" :alt="post.title" class="post-image card-img-top">
    <button @click="deletePost(post.id)" v-if="account.id == post.creatorId" class="btn btn-danger delete-button"
